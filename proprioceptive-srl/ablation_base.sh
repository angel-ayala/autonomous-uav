...
# deterministic model
REP_MODEL=(--is-srl --joint-optimization --model-ispr)
# stochastic model
...
for SEED in "${SEEDS[@]}"; do
  ...
  python learn_td3_mujoco.py --environment-id $ENV_NAME --seed $SEED --use-cuda "${REP_MODEL[@]}"
  sleep 2
  
  # SAC AmelPred base feature 32
  echo "Running learn_sac_mujoco.py: $ENV_NAME --seed $SEED ${REP_MODEL[@]}"
  python learn_sac_mujoco.py --environment-id $ENV_NAME --seed $SEED --use-cuda "${REP_MODEL[@]}"
  sleep 2
